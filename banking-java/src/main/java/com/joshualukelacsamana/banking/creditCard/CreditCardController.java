...
public class CreditCardController {
    ...
    @CrossOrigin(origins = "http://localhost:3000")
    @GetMapping("/creditCardBalance/{accountNumber}")
    public ResponseEntity<?> getCreditCardBalance(@PathVariable String accountNumber) {
        try {
            float creditCardBalance = creditCardService.getCreditCardBalance(accountNumber);
            return ResponseEntity.ok(creditCardBalance);
        } catch (IllegalStateException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An unexpected error occurred.");
        }
    }
